...
You are maintaining a Camber codebase. This file is written for you, the
...
Camber is a contract that ships a runtime. Server-rendered Python web
...
```bash
./gate.sh        # ruff + pytest with coverage + camber check
```
...
a thing to route around. `camber check` findings are called
**off-camber** and each one names the invariant it serves — read the
...
(OC-110); text is escaped unless you write `raw()`, and every `raw()` is
counted out loud; the file stays under its line ceiling (OC-140) — when
...
- A mutating route that renders is off-camber (OC-131). A genuine JSON
  endpoint carries `# camber: json-endpoint` with a reason in review.
...
- **Never write JavaScript.** The boost layer (`camber.js`) is complete
  and vendored. If a behavior seems to need JS, the behavior belongs on
  the server or in native HTML (`<details>`, `<dialog>`, `popover`,
  CSS `:has()`). A `.js` file from your hands is off-camber on sight
  (OC-120).
- **Never edit `ratchet.toml`.** `camber ratchet` is the only hand on
  the mechanism. Ceilings fall, floors rise; a loosened bound is caught
  against git history (OC-142) and will not survive review.
...
Run `./gate.sh`. If it is green and coverage rose, run `camber ratchet`
...
| word | meaning |
|------|---------|
| off-camber | a contract violation; the surface tilting against you |
...
| Tarmac | the design tokens; the poured surface |
| the road holds | `camber check` exit 0 |
