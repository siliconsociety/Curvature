...
def check_purposes(root: Path) -> list[Finding]:
    ...
    for path in walk_source(root, frozenset({".py"})):
        ...
        for node in ast.walk(tree):
            ...
            missing = purpose is None
            empty = isinstance(purpose, ast.Constant) and (
                purpose.value is None
                or (isinstance(purpose.value, str) and not purpose.value.strip())
            )
            if missing or empty:
                findings.append(Finding(
                    "ANOM-170", str(path.relative_to(root)), node.lineno,
                    "respond() without a non-empty purpose= (C-902): the chart needs its "
